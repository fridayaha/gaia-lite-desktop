...
import CommunityIcon from "~icons/ri/group-line";
...
const openDocs = () => {
  // docs 是独立 VitePress SPA，新标签打开
  window.open("/docs/", "_blank");
};

const openCommunity = () => {
  // 社区是 admin 内部路由，同标签跳转（新标签会重新初始化 router 守卫，落到首页）
...
<template>
  <div class="navbar bg-white shadow-xs shadow-[rgba(0,21,41,0.08)]">
    ...
    <div v-if="layout === 'vertical'" class="vertical-header-right">
      ...
      <!-- 资料中心 -->
      <span
        class="set-icon navbar-bg-hover hover:[&>svg]:animate-scale-bounce select-none"
        :title="t('nav.docs')"
        @click="openDocs"
      >
        <IconifyIconOffline :icon="BookIcon" />
      </span>
      <!-- 社区 -->
      <span
        class="set-icon navbar-bg-hover hover:[&>svg]:animate-scale-bounce select-none"
        ...
        @click="openCommunity"
      >
        <IconifyIconOffline :icon="CommunityIcon" />
      </span>
      <!-- 退出登录 -->

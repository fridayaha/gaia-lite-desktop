/**
 * 0.8.101 邮件服务商配置 multi-provider 验证脚本 — 在 ECS 上端到端测试
 *
 * 验证项：
 * 1. 邮件卡显示 el-table list（不再是 el-form singleton），空状态文案正确
 * 2. 新建 dialog 选 provider 时字段联动（smtp → smtp_host/port/encryption/username/password；
 *    aliyun/tencent/huawei → access_key_id/secret/region/from_email）
 * 3. 创建 4 个 provider 配置（每个一行）
 * 4. 测试连接按钮：4 个 provider 都会调用对应 SDK（假凭据必然探活失败，但应返回错误而非崩溃）
 * 5. 设为发码渠道：仅一行 is_active=true，激活其他行时上一行自动 deactivate
 * 6. 删除一行
 * 7. 截图最终状态
 *
 * 用法：node scripts/verify-0.8.101-email-multi-config.mjs
 */
...
const OUT_DIR = path.resolve("./tmp/verify-0.8.101");
...
const FAKE = {
  smtp: {
    smtp_host: "smtp.fake.example.com",
    smtp_port: 465,
    encryption: "ssl",
    username: "alerts@fake.example.com",
    password: "fake-smtp-password-not-real",
    from_name: "UA Alerts (Test)",
    daily_limit: 200,
    interval_seconds: 60,
  },
  aliyun: {
    ...
    region: "cn-hangzhou",
    from_email: "alerts@fake.example.com",
    from_name: "UA Aliyun (Test)",
    daily_limit: 200,
    interval_seconds: 60,
  },
  tencent: {
...
(async () => {
  ...
  try {
    // ===== Step 1: 跳转安全配置页 → 邮件卡显示 el-table list =====
    ...
    // 邮件卡应该有 el-table（不再是 el-form singleton）
    const emailCard = page.locator(".el-card").filter({ hasText: "邮件服务商配置" }).first();
    expect(await emailCard.count() > 0, "邮件卡标题「邮件服务商配置」存在");

    const emailTable = emailCard.locator(".el-table").first();
    expect(await emailTable.count() > 0, "邮件卡渲染 el-table（multi-config list 模式）");

    // 空状态文案
    const emptyText = await emailCard.locator(".el-table__empty-text").first().textContent().catch(() => "");
    expect(emptyText && emptyText.includes("暂无邮件服务商配置"), `空状态文案「暂无邮件服务商配置」(actual: ${emptyText?.trim()})`);

    // ===== Step 2: 新建 dialog 选 provider 时字段联动 =====
    log("step2", "→ 点击「新建」按钮打开 dialog");
    const newBtn = emailCard.locator("button").filter({ hasText: "新建" }).first();
    await newBtn.click();
    await page.waitForTimeout(500);
    const dialog = page.locator(".el-dialog").filter({ hasText: "新建邮件服务商" }).first();
    await dialog.waitFor({ state: "visible", timeout: 5000 });
    expect(await dialog.isVisible(), "新建 dialog 可见");

    // 选 smtp → 应出现 smtp_host 字段
    const smtpRadio = dialog.locator(".el-radio").filter({ hasText: "SMTP" }).first();
    await smtpRadio.click();
    await page.waitForTimeout(300);
    expect(await dialog.locator("label").filter({ hasText: "SMTP 主机" }).count() > 0 ||
           await dialog.locator(".el-form-item__label").filter({ hasText: "SMTP 主机" }).count() > 0,
       "选 SMTP provider → 出现 SMTP 主机 字段");

    // 切到 aliyun → 应出现 access_key_id 字段，smtp_host 字段消失
    ...
    // ===== Step 3: 通过 API 创建 4 个 provider 配置（更快、稳定）=====
    log("step3", "→ 通过 API 创建 4 个 provider 配置");
    ...
    for (const [provider, payload] of Object.entries(FAKE)) {
      const resp = await apiCtx.post("/api/manager/email-configs", {
    ...
    expect(createdIds.length === 4, `4 个 provider 都创建成功 (actual: ${createdIds.length})`);

    // reload page → table 应显示 4 行
    ...
    const emailCard2 = page.locator(".el-card").filter({ hasText: "邮件服务商配置" }).first();
    const tableRows = emailCard2.locator(".el-table__body-wrapper tr");
    const rowCount = await tableRows.count();
    expect(rowCount === 4, `表格显示 4 行配置 (actual: ${rowCount})`);
    await shot(page, "03-list-4-providers");

    // 验证 4 个 provider 都出现（顺序按 is_active desc, updated_at desc）
    const providerTags = await emailCard2.locator(".el-table .el-tag").allTextContents();
    const providerLabels = providerTags.map(s => s.trim()).filter(s => s.length > 0);
    expect(providerLabels.some(t => t.includes("SMTP")), "表格中有 SMTP tag");
    ...
    for (const { provider, id } of createdIds) {
      const testResp = await apiCtx.post(`/api/manager/email-configs/${id}/test`, { headers: authHeaders });
    ...
    // ===== Step 5: 设为发码渠道 — 激活唯一性 =====
    log("step5", "→ POST /api/manager/email-configs/{id}/activate 激活 aliyun 行");
    const aliyunRow = createdIds.find(r => r.provider === "aliyun");
    const actResp = await apiCtx.post(`/api/manager/email-configs/${aliyunRow.id}/activate`, { headers: authHeaders });
    ...
    const listResp = await apiCtx.get("/api/manager/email-configs", { headers: authHeaders });
    ...
    const act2Resp = await apiCtx.post(`/api/manager/email-configs/${tencentRow.id}/activate`, { headers: authHeaders });
    expect(act2Resp.ok(), `activate tencent 返回 200`);
    const list2 = await (await apiCtx.get("/api/manager/email-configs", { headers: authHeaders })).json();
    ...
    // ===== Step 6: 删除一行 =====
    log("step6", "→ DELETE 邮件配置（删除 huawei 行）");
    const huaweiRow = createdIds.find(r => r.provider === "huawei");
    const delResp = await apiCtx.delete(`/api/manager/email-configs/${huaweiRow.id}`, { headers: authHeaders });
    expect(delResp.status() === 204, `删除 huawei 返回 204 (status=${delResp.status()})`);

    const list3 = await (await apiCtx.get("/api/manager/email-configs", { headers: authHeaders })).json();
    expect(list3.length === 3, `删除后剩 3 行 (actual: ${list3.length})`);
    ...
    // 清理：删除剩余 3 行，让 DB 回到初始状态
    log("cleanup", "→ 清理：删除剩余 3 行测试配置");
    for (const c of list3) {
      await apiCtx.delete(`/api/manager/email-configs/${c.id}`, { headers: authHeaders });
    }
    const list4 = await (await apiCtx.get("/api/manager/email-configs", { headers: authHeaders })).json();

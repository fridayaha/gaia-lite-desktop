"""DVP trivial baselines (DESIGN.md §3 Task Validity, §4 trivial agent).
...
Read-path baselines:
  ...
  - enumeration: return a list of all possible ids (must be rejected by
    anti-enumeration kinds like count_eq).
"""
...
READ_BASELINES = {
    ...
    "L4": ["do_nothing", "dump_all"],  # count_eq: dump_all gives wrong count
    "L5": ["do_nothing", "dump_all"],
    "L6": ["do_nothing", "dump_all"],
    "L7": ["do_nothing", "dump_all", "enumeration"],
    "L8": ["do_nothing", "dump_all"],  # count_eq: dump_all gives wrong count
    "L9": ["do_nothing", "dump_all"],
    "L10": ["do_nothing", "dump_all"],
    "L11": ["do_nothing", "dump_all"],
    "L12": ["do_nothing", "dump_all"],  # count_eq
    "L13": ["do_nothing", "dump_all", "enumeration"],
    "L14": [],  # snapshot_diff: Tier3 XFAIL, no trivial baseline
}

...
class BetterleaksScannerAdapter:
    """Betterleaks primary secret scanner adapter.

    默认不启用（HUB_BETTERLEAKS_ENABLED=false）。
    使用 betterleaks dir 命令扫描临时目录中的版本内容。
    """
    ...
    def __init__(self, enabled: bool | None = None):
        self._enabled = (
            enabled if enabled is not None else settings.betterleaks_enabled
        )
        self._bin = settings.betterleaks_bin or BETTERLEAKS_BIN
        self._timeout = settings.betterleaks_timeout_seconds or BETTERLEAKS_TIMEOUT
    ...
    def _run_betterleaks(self, tmpdir: str) -> list[dict]:
        ...
        if settings.betterleaks_config:
            cmd.extend(["-c", settings.betterleaks_config])
        result = subprocess.run(
            cmd,
        ...
        if result.returncode == 0:
            ...
            try:
                ...
                raise RuntimeError(
                    "betterleaks report parse failed: invalid JSON"
                )
            except OSError:
                raise RuntimeError(
                    "betterleaks report not found or unreadable"
                )
        else:
            raise RuntimeError(
                f"betterleaks exited with code {result.returncode}: "
                + (result.stderr[:200] if result.stderr else "unknown error")
            )

    def _normalize(self, raw: dict) -> dict:
        rule_id = raw.get("RuleID", raw.get("ruleId", "unknown"))
        risk_type = f"ext:betterleaks:{rule_id}"
        ...
        if match is not None:
            evidence["redacted_match_length"] = len(str(match))
    ...
    def scan(self, version: HubItemVersion) -> list[dict]:
        ...
        if not shutil.which(self._bin):
            ...
            raise RuntimeError(
                f"betterleaks binary not found: {self._bin}"
            )

        with tempfile.TemporaryDirectory(prefix="hub_betterleaks_") as tmpdir:
            self._write_version_files(version, tmpdir)
            raw_findings = self._run_betterleaks(tmpdir)
